...
void pipeline_t::execute(unsigned int lane_number) {
   ...
   if (Execution_Lanes[lane_number].ex[depth].valid) {
      ...
      if (IS_MEM_OP(PAY.buf[index].flags)) {
         ...
         if (IS_LOAD(PAY.buf[index].flags)) {
            ...
            // FIX_ME #13
            // If the load hit AND the load has a destination register (*see footnote below):
            ...
            //    AND the load has a destination register (*see footnote below):
	    ...
	    //
	    // (*footnote) There are rare instances of loads with r0 as their destination register. The RISCV ISA defines logical
	    // register r0 to be hardwired to the value 0. Thus, the Decode stage interprets a destination register of r0 to mean
	    ...
	    // check the existence (validity) of a destination register.
         ...
         // FIX_ME #14
         // If the ALU type instruction has a destination register (*see footnote below),
         ...
         // Tips:
         // 1. At this point of the code, 'index' is the instruction's index into PAY.buf[] (payload).
         // 2. If the ALU type instruction has a destination register (*see footnote below), then write the doubleword value of the
         ...
         //    separately); see the comments in file payload.h regarding referencing a value as a single doubleword.
	 //
	 // (*footnote) Conditional branches execute in the same lanes as simple ALU instructions, and they don't have destination registers.
   ...
   if (depth > 0) {
	 ...
	 //
         ...
         // 3. If the instruction has a destination register AND it is not a load AND it is not an AMO instruction, then:
